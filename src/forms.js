...
window.Forms = (() => {
  ...
  const DIST_TYPES = [
    ...
    { value: 'ренто_рс', text: 'Ренто на р/с (по ссылке гостем)',
      // тип_кассы — канон 'р/с ООО Сингуляр' (решение Морган): под него
      // сверён май и настроена формула ДДС. Раньше писали 'р/с' →
      // нормализовано скриптом fix_kassa_label.py; теперь пишем канон
      // сразу, чтобы новые строки не уезжали из ДДС.
  ...
  function openЗаселение(opts) {
    ...
    const KASSA_TO_DIST = {
      // 'р/с' — историческая метка до нормализации (fix_kassa_label.py);
      // 'р/с ООО Сингуляр' — текущий канон. Обе → ренто_рс при реконструкции.
      'р/с': 'ренто_рс', 'р/с ООО Сингуляр': 'ренто_рс',
      'безнал': 'ренто_безнал', 'карта физлица': 'ренто_карта',
    };
