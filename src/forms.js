...
window.Forms = (() => {
  ...
  function openКарточкаКвартиры(opts) {
    ...
    // Все секции закрыты, открываются по тапу (фаундер 28.08 —
    // отменяет «опись раскрыта сразу» от 27.08).
    const secOpen = {};
    ...
    // Опись v3 (фаундер 27.08): заполняется инлайн — как чек-лист на
    // канбане (с 28.08 секция закрыта по умолчанию, как остальные).
    // «+ Комната» добавляет комнату во все
