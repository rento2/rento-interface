...
window.Screens = (() => {
  ...
  function connect(opts) {
    ...
    app.append(h('div', { class: 'auth-wrap' },
      h('div', { class: 'card auth-card' },
        ...
        errorBox,
        h('p', { class: 'caption' },
          'Интерфейс читает данные из Google Sheets — подключите ' +
          'аккаунт, у которого есть доступ к боевому файлу.'),
        btn)));
  }
  ...
  function main(opts) {
    ...
    // Доступен всем сотрудникам: менеджер заводит задачу и двигает статус
    // (решение фаундера). Отдельный блок — это не финансовая операция.
    actions.append(h('span', { class: 'eyebrow eyebrow-sub' }, 'СЕРВИС'));
    actions.append(h('div', { class: 'cards-grid' },
      card('Задача по сервису', 'задача_сервис', '💬'),
      card('Задачи — список', 'задачи_сервис', '✅')));
